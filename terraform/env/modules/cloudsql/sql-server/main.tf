...
# Create a custom database (if a database name is provided) even if `enable_default_db` is false
resource "google_sql_database" "custom_db" {
  count    = length(var.database_name) > 0 ? 1 : 0  # Create the custom DB if a name is provided
